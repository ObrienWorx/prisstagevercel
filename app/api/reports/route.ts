...
export async function POST(req: NextRequest) {
  ...
  await connectDB();
  const { title, slug, content, featuredImage, category, sector, product, upsellTicker, publishStatus, metaTitle, metaDescription, metaImage } = await req.json();
  if (!title) return errorResponse('Title is required');
  const finalSlug = slug ? slugify(slug) : slugify(title);
  if (await Report.findOne({ slug: finalSlug })) return errorResponse('Slug already exists');
  const report = await Report.create({ title, slug: finalSlug, content, featuredImage, category: category || null, sector: sector || null, product: product || null, upsellTicker, publishStatus: publishStatus || 'draft', metaTitle, metaDescription, metaImage });
