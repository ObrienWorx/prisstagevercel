...
export async function GET(req: NextRequest) {
  ...
  await connectDB();
  const portfolios = await Portfolio.find({ subscriberId }).lean();
...
export async function POST(req: NextRequest) {
  ...
  if (!name?.trim()) return errorResponse('Portfolio name is required', 400);
  await connectDB();
  const portfolio = await Portfolio.create({ subscriberId, name: name.trim(), stocks: [] });
  return successResponse(portfolio, 'Created', 201);
}
