...
export async function POST(req: NextRequest) {
  try {
    ...
    // Duplicate check for unlock-ticker: same email OR phone already submitted
    if (normalisedSource === 'unlock-ticker') {
      const existing = await LeadSubmission.findOne({
        source: 'unlock-ticker',
        $or: [{ email: email.toLowerCase().trim() }, { phone }],
      });
      if (existing) {
        return successResponse({ alreadyExists: true }, 'Already unlocked', 200);
      }
    }
