...
export default function ReportCategoriesPage() {
  ...
  const load = useCallback(async () => { setLoading(true); try { const r = await fetch('/api/report-categories', { headers: h }); const d = await r.json(); if (d.success) setItems(d.data); } finally { setLoading(false); } }, []);
  useEffect(() => { load(); }, [load]);
  ...
  const save = async () => {
    ...
    try {
      const url = editing ? `/api/report-categories/${editing._id}` : '/api/report-categories';
  ...
  const remove = async () => {
    ...
    try { const r = await fetch(`/api/report-categories/${del._id}`, { method: 'DELETE', headers: h }); const d = await r.json(); if (d.success) { flash('Deleted'); setDel(null); load(); } } finally { setSaving(false); }
  };
  return (
    <div>
      <div className="page-header">
        <div className="page-header-text"><h4>Report Categories</h4><p>Categories used only for Research Reports</p></div>
      ...
      <div className="card">
        {loading ? <div className="text-center p-5"><div className="spinner-border text-primary" /></div>
          : items.length === 0 ? <div className="empty-state"><div className="empty-icon">📊</div><p>No report categories yet.</p></div>
          : <div className="table-responsive"><table className="table"><thead><tr><th>Name</th><th>Slug</th><th>Status</th><th>Actions</th></tr></thead><tbody>
            {items.map((x) => (<tr key={x._id}><td className="fw-semibold">{x.name}</td><td><code style={{ fontSize: 12, background: '#f1f5f9', padding: '2px 6px', borderRadius: 4 }}>{x.slug}</code></td><td><span className={`badge ${x.status === 'active' ? 'bg-success' : 'bg-secondary'}`}>{x.status}</span></td><td><button className="btn btn-sm btn-outline-primary me-1" onClick={() => openEdit(x)}>Edit</button><button className="btn btn-sm btn-outline-danger" onClick={() => setDel(x)}>Del</button></td></tr>))}
          </tbody></table></div>}
      </div>
      {showModal && (
        <div className="modal d-block" style={{ backgroundColor: 'rgba(15,23,42,0.55)' }}>
          <div className="modal-dialog modal-lg modal-dialog-scrollable"><div className="modal-content">
            <div className="modal-header"><h5 className="modal-title">{editing ? 'Edit' : 'New'} Report Category</h5><button className="btn-close" onClick={() => setShowModal(false)} /></div>
